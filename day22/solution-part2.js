...
const getNumOfFallenBricks = bricks => {
  const setOfFallenBricks = new Set();
  const maxZ = Math.max(...bricks.map(b => Math.max(b.start[2], b.end[2])));
  ...
  for (let z = 0; z <= maxZ; z++) {
    ...
    brickInLine.forEach(brickIndex => {
      while (canBrickFall(bricks, brickIndex)) {
        setOfFallenBricks.add(brickIndex);
        bricks[brickIndex].start[2] = bricks[brickIndex].start[2] - 1;
        bricks[brickIndex].end[2] = bricks[brickIndex].end[2] - 1;
      }
    });
...
const getAnswer = strings => {
  ...
  const res = bricks.reduce((acc, curr, i) => {
    if (i % 10 === 0) console.log('i', i);

    if (!canBrickBeRemoved(bricks, i)) {
      let bricksWithoutCurrent = JSON.parse(JSON.stringify(bricks));
      bricksWithoutCurrent[i] = {start: [-1, -1, -1], end: [-1, -1, -1]};
      bricksWithoutCurrent = bricksWithoutCurrent.sort(
        (a, b) => Math.min(a.end[2], a.start[2]) - Math.min(b.end[2], b.start[2])
      );
      const numOfFallenBricks = getNumOfFallenBricks(bricksWithoutCurrent);
      acc += numOfFallenBricks;
    }
